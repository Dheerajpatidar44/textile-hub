    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: C.bg, minHeight: '90vh' }} className="pt-2 pb-16">
      <div className="max-w-[90rem] mx-auto px-6 sm:px-8 lg:px-14">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="w-full md:w-[280px] flex-shrink-0 md:sticky md:top-[110px] p-4 md:p-6 rounded-2xl shadow-sm" style={{ background: 'white', border: `1px solid ${C.border}` }}>
            <h3 className="hidden md:block" style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 700, color: C.primary, marginBottom: 16, paddingBottom: 12, borderBottom: `1px solid ${C.border}` }}>
              Categories
            </h3>
            {/* Wrapped pills on mobile, vertical list on desktop */}
            <div className="flex flex-row flex-wrap md:flex-col md:flex-nowrap gap-2 md:gap-1.5 overflow-x-hidden md:overflow-y-auto md:max-h-[60vh] md:pr-2 pb-1 md:pb-0" style={{ scrollbarWidth: 'thin', scrollbarColor: `${C.primary} transparent` }}>
              {["All", ...categories].map((cat) => {
                ...
                return (
                  <button
                    ...
                    className="text-left whitespace-nowrap md:whitespace-normal w-auto md:w-full px-5 md:px-4 py-2 md:py-2.5 rounded-full md:rounded-xl text-[12px] md:text-[13px] font-bold tracking-wide transition-all duration-200 cursor-pointer flex items-center justify-between flex-shrink-0"
                    style={{
                      background: active ? 'rgba(59, 71, 85, 0.08)' : 'transparent',
                      color: active ? C.primary : C.stone,
                      border: 'none',
                    }}
                    onMouseEnter={e => { if (!active) { e.currentTarget.style.background = 'rgba(59, 71, 85, 0.04)'; e.currentTarget.style.color = C.primary; }}}
                    onMouseLeave={e => { if (!active) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = C.stone; }}}
                  >
                    <span>{cat}</span>
                    {active && <div className="hidden md:block" style={{ width: 6, height: 6, borderRadius: '50%', background: C.accent }} />}
                  </button>
                );
              })}
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredProducts.map((product, index) => (
                  <motion.div
                    ...
                    className="text-left group rounded-2xl bg-white border border-[#E2E8F0] shadow-sm hover:shadow-lg hover:border-[#D4B26F]/40 transition-all duration-300 overflow-hidden"
                  >
                    <div style={{ height: 280, overflow: 'hidden', position: 'relative' }}>
                      ...
                      />
                      <div className="absolute inset-0 bg-[#3B4755]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-1000 pointer-events-none" />
                    </div>
                    <div style={{ padding: '24px' }}>
                      ...
                      <h3 style={{ fontSize: 16, fontWeight: 700, color: C.primary, margin: '0 0 20px', lineHeight: 1.4, fontFamily: "'Playfair Display', serif" }}>
                        {product.name}
                      </h3>
                      <button
                        style={{
