...
const reviews = [
  { text: "KUMAVATI Textile House has been our trusted partner for years. The quality, and B2B quotation services are unmatched in the entire industry.", name: "Rajesh Sharma", role: "Retailer, Delhi", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80" },
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: C.cream, minHeight: '90vh' }} className="pt-2 pb-16">
      <div className="max-w-[90rem] mx-auto px-6 lg:px-14 pt-0">
        ...
        <div className="text-center mb-10 mt-2">
          <span className="text-[10px] font-bold tracking-[0.25em] text-[#D4B26F] uppercase block mb-1">REVIEWS</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review, idx) => (
            <motion.div
              ...
              className="bg-white rounded-2xl border border-[#E2E8F0] p-8 text-left shadow-sm flex flex-col justify-between hover:shadow-lg hover:border-[#D4B26F]/40 transition-all duration-300"
            >
              <div className="space-y-4">
                ...
                {/* Quote Text */}
                <p className="text-[13px] text-[#5E6A7E] leading-relaxed font-semibold italic">
                  "{review.text}"
                </p>
              </div>

              {/* User Profile Footer */}
              <div className="flex items-center gap-3.5 pt-6 mt-6 border-t border-[#E2E8F0]/45">
                {review.avatar ? (
                  <img
                    src={review.avatar}
                    alt={review.name}
                  ...
                  <div 
                    ...
                    style={{ background: 'rgba(59, 71, 85, 0.08)', color: C.accent }}
                  >
                    <User size={16} />
                  </div>
                )}
                <div>
                  <h4 className="text-[12.5px] font-bold text-[#3B4755] m-0">{review.name}</h4>
                  <p className="text-[11px] text-[#5E6A7E] font-medium m-0">{review.role}</p>
                </div>
              </div>
            </motion.div>
          ))}
