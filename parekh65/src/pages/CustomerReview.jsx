    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: C.bg, minHeight: '90vh' }} className="pt-2 pb-16">
      <div className="max-w-[90rem] mx-auto px-6 lg:px-14 pt-0">
        ...
        <div className="text-center mb-10 mt-2">
          <span className="text-[10px] font-bold tracking-[0.25em] text-[#C8966A] uppercase block mb-1">REVIEWS</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {reviews.map((review, idx) => (
            <motion.div
              ...
              className="bg-white rounded-[24px] border border-[#E8DDD0] p-8 text-left shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow duration-300"
            >
              <div className="space-y-4">
                {/* 5 Stars */}
                <div className="flex items-center gap-0.5 text-[#C8966A]">
                  {[...Array(5)].map((_, i) => (
                ...
                {/* Quote Text */}
                <p className="text-[13px] text-[#7A6558] leading-relaxed font-semibold italic">
                  "{review.text}"
                </p>
              </div>

              {/* User Profile Footer */}
              <div className="flex items-center gap-3.5 pt-6 mt-6 border-t border-[#E8DDD0]">
                {review.avatar ? (
                  <img
                    src={review.avatar}
                    alt={review.name}
                ...
                <div>
                  <h4 className="text-[12.5px] font-bold text-[#6B4226] m-0">{review.name}</h4>
                  <p className="text-[11px] text-[#7A6558] font-medium m-0">{review.role}</p>
                </div>
              </div>
            </motion.div>
          ))}
