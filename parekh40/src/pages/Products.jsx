...
export default function Products() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: C.bg, minHeight: '100vh' }} className="pt-[40px] pb-20">
      <div className="max-w-[90rem] mx-auto px-6 sm:px-8 lg:px-14">

        {/* Page Title */}
        <div className="text-center mb-10">
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(36px, 5vw, 48px)', fontWeight: 600, color: C.primary, margin: '0 0 16px', letterSpacing: '0.02em' }}>
            Our Collections
          </h1>
            ...
            {["All", ...categories].map((cat) => {
              ...
              return (
                <button
                  ...
                  className="px-6 py-2.5 rounded-full text-[13px] font-medium tracking-wide transition-all duration-200 cursor-pointer whitespace-nowrap border shadow-sm"
                  style={{
                    background: active ? C.primary : 'white',
                    color: active ? 'white' : C.stone,
                    borderColor: active ? C.primary : C.border,
                  }}
                  onMouseEnter={e => { if (!active) { e.currentTarget.style.background = C.sand; e.currentTarget.style.color = C.primary; }}}
                  onMouseLeave={e => { if (!active) { e.currentTarget.style.background = 'white'; e.currentTarget.style.color = C.stone; }}}
                >
                  {cat}
                </button>
              );
        ...
        <div className="w-full">
          <div className="flex items-center justify-between mb-8 pb-4 border-b" style={{ borderColor: C.border }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 700, color: C.soil, margin: 0 }}>
              {activeCategory === "All" ? "All Products" : activeCategory}
            </h2>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', background: 'white', borderRadius: 20, border: `1px solid ${C.border}` }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {filteredProducts.map((product, index) => (
                <motion.div
                  ...
                  className="text-left group rounded-[16px]"
                ...
                >
                  ...
                  <div style={{ padding: '20px' }}>
                    ...
                    <h3 style={{ fontSize: 15, fontWeight: 700, color: C.soil, margin: '0 0 16px', lineHeight: 1.4, fontFamily: "'Playfair Display', serif" }}>
                      {product.name}
                    </h3>
                    <button
                      style={{
