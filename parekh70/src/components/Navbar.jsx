...
const C = {
  ...
  primaryLight: '#2E4070',  // Medium Navy
  primaryDark: '#0D1828',   // Very Dark Navy
  accent: '#D4A842',        // Rich Gold
  crimson: '#8B1A2D',       // Deep Burgundy
  crimsonLight: '#B52540',  // Lighter Crimson
  bg: '#F8F7F4',            // Off-White Background
  border: '#E2DDDA',        // Light Border
  stone: '#5A5560',         // Muted Neutral Text
};
...
export default function Navbar() {
  ...
  const navTextColor = isTransparent ? 'rgba(255,255,255,0.92)' : 'rgba(27, 42, 74, 0.85)';
  const navActiveColor = isTransparent ? '#FFFFFF' : C.primary;
  const navHoverColor = isTransparent ? '#D4A842' : '#D4A842';
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: isTransparent ? 'transparent' : 'rgba(248, 247, 244, 0.97)',
          borderBottom: isTransparent ? 'none' : `1px solid ${C.border}`,
          boxShadow: isTransparent ? 'none' : '0 4px 24px rgba(27, 42, 74, 0.06)',
        ...
        <div className="max-w-[95rem] mx-auto px-4 sm:px-8 lg:pl-6 lg:pr-14 flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-3 group shrink-0 text-left">
            <div style={{
              ...
              background: isTransparent ? 'rgba(255,255,255,0.12)' : 'rgba(27,42,74,0.08)',
              borderRadius: '50%',
              border: isTransparent ? '1px solid rgba(255,255,255,0.25)' : `1px solid ${C.border}`,
              transition: 'all 0.3s ease',
            }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                <path d="M12 2L2 7l10 5 10-5-10-5z" stroke={isTransparent ? '#D4A842' : C.accent} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M2 17l10 5 10-5" stroke={isTransparent ? '#D4A842' : C.accent} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M2 12l10 5 10-5" stroke={isTransparent ? 'rgba(255,255,255,0.7)' : C.crimson} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <div className="flex flex-col">
              <span
                className="text-[20px] font-bold leading-none tracking-[0.04em] uppercase whitespace-nowrap"
                style={{ fontFamily: "'Playfair Display', serif", color: isTransparent ? '#FFFFFF' : C.primary, transition: 'color 0.3s ease' }}
              >
                Meraki Ethnic
              </span>
              <span
          ...
          <nav className="hidden lg:flex items-center gap-x-1">
            {directNavItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  style={{
                    color: active ? (isTransparent ? '#D4A842' : C.primary) : navTextColor,
                  }}
                  onMouseEnter={e => { e.currentTarget.style.color = navHoverColor; }}
                  onMouseLeave={e => { e.currentTarget.style.color = active ? (isTransparent ? '#D4A842' : C.primary) : navTextColor; }}
                >
                  {item.name}
                  <span 
            ...
            >
              <button
                ...
                style={{
                  color: isDropdownActive() ? (isTransparent ? '#D4A842' : C.primary) : navTextColor,
                }}
                onMouseEnter={e => { e.currentTarget.style.color = navHoverColor; }}
                onMouseLeave={e => { e.currentTarget.style.color = isDropdownActive() ? (isTransparent ? '#D4A842' : C.primary) : navTextColor; }}
              >
                <span>PAGES</span>
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  ...
                  >
                    {/* Visual accent bar */}
                    <div className="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-[#1B2A4A] via-[#D4A842] to-[#8B1A2D]" />
                      ...
                      {dropdownNavItems.map((item) => {
                        ...
                        return (
                          <Link
                            ...
                            style={{
                              background: active ? 'rgba(27, 42, 74, 0.05)' : 'transparent',
                            }}
                            onMouseEnter={e => { if (!active) e.currentTarget.style.background = 'rgba(212, 168, 66, 0.08)'; }}
                          ...
                          >
                            ...
                            <div 
                              ...
                              style={{
                                background: active ? '#1B2A4A' : 'rgba(27, 42, 74, 0.04)',
                                borderColor: active ? '#1B2A4A' : '#E2DDDA',
                            ...
                            {/* Label */}
                            <span 
                              className="text-[12px] font-bold tracking-wider transition-colors group-hover/item:text-[#D4A842]"
                              style={{ color: active ? C.primary : '#0D1828' }}
                            >
          ...
          <div className="hidden lg:flex items-center gap-4 shrink-0">
            <Link
              ...
              style={{
                background: isTransparent ? 'rgba(212, 168, 66, 0.9)' : C.primary,
                border: isTransparent ? '1px solid rgba(255,255,255,0.3)' : 'none',
              }}
              onMouseEnter={e => { e.currentTarget.style.background = '#D4A842'; }}
              onMouseLeave={e => { e.currentTarget.style.background = isTransparent ? 'rgba(212, 168, 66, 0.9)' : C.primary; }}
            >
              TRADE ENQUIRY
            </Link>
          </div>
                ...
                {directNavItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      style={{
                        background: active ? 'rgba(27, 42, 74, 0.06)' : 'transparent',
