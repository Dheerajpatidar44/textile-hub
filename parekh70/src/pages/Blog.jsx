        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {posts.map((post, idx) => (
            <motion.div 
              ...
              className="bg-white rounded-[24px] border border-[#E2DDDA] overflow-hidden text-left hover:shadow-md transition-shadow cursor-pointer animate-fade-in"
            >
              <div className="w-full aspect-video bg-[#F8F7F4]">
                <img src={post.image} alt={post.title} className="w-full h-full object-cover" loading="lazy" />
              </div>
              <div className="p-6">
                <span className="text-[10px] text-[#5A5560] uppercase tracking-wider block mb-2 font-bold">{post.date}</span>
                <h3 className="text-[16px] font-bold text-[#1B2A4A] mb-4 line-clamp-2 leading-snug font-serif" style={{ fontFamily: "'Playfair Display', serif" }}>{post.title}</h3>
                <span className="text-[11px] font-bold text-[#D4A842] tracking-widest uppercase flex items-center gap-1">
