...
const reviews = [
  { text: "Kaaya Fabrics has been our trusted partner for years. The quality, and B2B quotation services are unmatched in the entire industry.", name: "Rajesh Sharma", role: "Retailer, Delhi", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80" },
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: C.cream, minHeight: '90vh' }} className="pt-2 pb-16">
      <div className="max-w-[90rem] mx-auto px-6 lg:px-14 pt-0">
        ...
        <div className="text-center mb-10 mt-1">
          <span className="text-[10px] font-bold tracking-[0.25em] text-[#E5A391] uppercase block mb-1">REVIEWS</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review, idx) => (
            <motion.div
              ...
              className="bg-white rounded-xl border border-[#EFE9E5] p-8 text-left shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow duration-300"
            >
              <div className="space-y-4">
                {/* 5 Stars */}
                <div className="flex items-center gap-0.5 text-[#E5A391]">
                  {[...Array(5)].map((_, i) => (
                ...
                {/* Quote Text */}
                <p className="text-[13px] text-[#6B5B56] leading-relaxed font-medium italic">
                  "{review.text}"
                </p>
              </div>

              {/* User Profile Footer */}
              <div className="flex items-center gap-3.5 pt-6 mt-6 border-t border-[#EFE9E5]/45">
                {review.avatar ? (
                  <img
                    src={review.avatar}
                    alt={review.name}
                ...
                <div>
                  <h4 className="text-[12.5px] font-bold text-[#D28D7A] m-0">{review.name}</h4>
                  <p className="text-[11px] text-[#6B5B56] font-medium m-0">{review.role}</p>
                </div>
              </div>
            </motion.div>
          ))}
