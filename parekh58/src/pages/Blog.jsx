        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {posts.map((post, idx) => (
            <motion.div 
              ...
              className="bg-white rounded-2xl border border-[#F0E5E9] overflow-hidden text-left hover:shadow-md transition-shadow cursor-pointer animate-fade-in"
            >
              <div className="w-full aspect-video bg-[#FAF5F7]">
                <img src={post.image} alt={post.title} className="w-full h-full object-cover" loading="lazy" />
              </div>
              <div className="p-6">
                <span className="text-[10px] text-[#5F5558] uppercase tracking-wider block mb-2 font-bold">{post.date}</span>
                <h3 className="text-[16px] font-bold text-[#7E1242] mb-4 line-clamp-2 leading-snug font-serif">{post.title}</h3>
                <span className="text-[11px] font-bold text-[#C17D91] tracking-widest uppercase flex items-center gap-1">
